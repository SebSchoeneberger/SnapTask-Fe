  ...
  return (
    <>
      <dialog id="snapAdvisor" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box bg-base-100 rounded-2xl min-w-[1200px] h-full">
                ...
                <div className="flex flex-col gap-2 flex-grow overflow-y-auto p-4">
                  {messages.map((msg, index) => (
                    <div key={index} className={`chat ${msg.role === "assistant" ? "chat-start" : "chat-end"}`}>
                      ...
                      <div
                        className={`chat-bubble ${msg.role === "assistant" ? "bg-neutral text-neutral-content" : "bg-neutral-content text-neutral"}`}>
                        {msg.content}
                      </div>
                    </div>
                  ))}
