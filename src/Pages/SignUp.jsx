...
function SignUp() {
  ...
  const { signUp, user, storedPath } = useContext(AuthContext);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await signUp(data);
      navigate("/dashboard");
      toast.success("Registration Successful. Welcome to SnapTask!");
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-[500px_minmax(0,_1fr)] min-h-screen">
      <div className="p-8 flex flex-col items-start">
        ...
        <div className="divider divider-neutral text-xs mb-6">Or, sign in with email</div>

        <form className="w-full h-auto flex flex-col gap-3" onSubmit={handleSubmit(onSubmit)}>
          ...
          <label className="input input-bordered flex items-center gap-2 mb-6">
            ...
            <input type="text" placeholder="Last Name" {...register("lastName", { required: "Last name is required" })} className="grow " />
          </label>
      ...
      <div className="bg-gradient-to-r from-base-100 to-primary h-full flex items-center justify-center">
        <img className="max-w-screen-md" src={illustrationSignup} alt="Illustration of a woman and charts" />
      </div>
    </div>
  );
}
