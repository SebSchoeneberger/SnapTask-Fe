...
function Login() {
  ...
  const { storedPath, login, user } = useContext(AuthContext);
  ...
  if (user)
    ...
    else if (user.role === "staff") return <Navigate to={(storedPath && storedPath != "/login") || "/staff/areas"} />;
    ...
    <div className="grid grid-cols-[500px_minmax(0,_1fr)] max-md:inline">
      <div className="p-8 flex flex-col items-start min-h-screen">
        ...
        <form className="w-full h-auto flex flex-col gap-3" onSubmit={handleSubmit(onSubmit)}>
          ...
          <Link className="text-sm hover:text-blue-500 hover:underline inline-flex">Forgot Password?</Link>
      ...
      <div className="bg-gradient-to-r from-base-100 to-primary h-full flex items-center justify-center max-md:hidden">
        <img className="max-w-screen-md max-xl:w-4/5 max-md:hidden" src={illustrationSignin} alt="Illustration of a woman and charts" />
      </div>
    </div>
  );
}
